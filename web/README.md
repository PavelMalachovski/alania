...
## Без JS — сознательно, а не пока не дошли руки

На странице нет ни одного `<script>` (`test_no_javascript`), и это не
временное упрощение: страница — готовый экспорт React-бандла от Ланы, где
JS отвечал за три вещи. Все три сделаны заново на голом HTML/CSS, рантайм
выброшен целиком.
...
- **Аккордеон FAQ.** Шесть `<details class="faq-item" name="faq">`
  ...
  одного, никакого слушателя кликов не нужно. Первый вопрос раскрыт
  атрибутом `open`. Раньше это была отдельная дизайн-система
  (`LanaLeonovichDesignSystem` из бандла) — теперь два HTML-атрибута.
...
Если возникнет соблазн вернуть сюда JS (аналитику, форму, что угодно) —
имейте в виду, что `test_no_javascript` зафиксирован как требование, а не
случайность: тест придётся осознанно снять, а не обходить его костылём.
...
Дело не в скорости загрузки, хотя self-hosted и с этим лучше. П. 12.1
Политики конфиденциальности утверждает, что сайт не передаёт данные
посетителей третьим лицам. Google Fonts — технически сторонний запрос:
