...
- `handlers/` — по одному роутеру на файл, собираются в `setup_routers()`;
  ...
  - `reply.py` — хендлеры нижней клавиатуры (`keyboards/reply.py`), 7 кнопок:
    ...
    «Записаться» → `open_calendar(..., send=True)` (оба + `clear_screen` до
    и `track_screen` после — новый экран заменяет прежнее окно); «Вопрос
    Лане» → текст со ссылкой (`ask_lana_kb()`). Каждый хендлер сперва удаляет
...
- `keyboards/reply.py` — `main_reply_kb()`: постоянная `ReplyKeyboardMarkup`
  ...
  (Мои записи), `BTN_BOOK` (Записаться), `BTN_ASK` (Вопрос Лане, растянут на
  всю ширину нижним рядом). Раскладка — 2 колонки, последний ряд одна кнопка.
