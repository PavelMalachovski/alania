...
- `handlers/` — по одному роутеру на файл, собираются в `setup_routers()`;
  ...
  - `start.py` — `/start`, `/id`, возврат в главное меню. `/start` через
    ...
    («В меню»/«Назад» из под-экранов) идёт через `render_screen()` — возвращает
    приветствие **правкой того же окна на месте** (новое сообщение не шлётся,
    иначе копятся дубликаты; нижняя клавиатура держится на этом же окне).
    `MAIN_MENU_TEXT` — без декоративной шапки `○─── ☾ ───○` и без строки
    «Быстрые действия».
