...
- Рассылка `/broadcast` использует `copy_message` — работает с **любым** типом
  ...
  с паузой ~0.15с между отправками и обработкой `TelegramRetryAfter`. После
  доставки каждому пользователю бот пересобирает низ чата (`_restore_bottom`:
  `reset_keyboard` + `show_screen(MAIN_MENU_TEXT)`) — иначе сообщение Ланы
  падает ПОД 🤍-якорь клавиатуры и старое окно; нужно наоборот, чтобы оно
  оказалось над сердечком, а меню осталось внизу. Отсюда и пауза 0.15с:
  отправок на человека стало три вместо одной. Побочный эффект принят: если
  клиент читал раздел, вместо него он увидит приветствие.
