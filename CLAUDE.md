...
- `ui.py` — хелперы гигиены сообщений, на них держится вся уборка переписки
  ...
  нужно оборачивать их в try/except. Плюс «единое окно на чат»:
  `show_screen(bot, chat_id, text, reply_markup=None)` удаляет ранее
  показанный экран этого чата и шлёт новый, запоминая его id в модульном
  `_last_screen: dict[chat_id → message_id]` (in-memory, после рестарта
  просто забывается — уборка косметическая). `track_screen(chat_id, message)`
  запоминает уже отправленное сообщение как текущий экран (для случаев, где
  сообщение шлёт не `show_screen`, а `open_calendar`/`open_my_bookings`);
  `clear_screen(bot, chat_id)` удаляет и забывает текущий экран. За счёт
  этого `/start` и «В меню» убирают предыдущее окно, а не копят меню в чате.
- `handlers/` — по одному роутеру на файл, собираются в `setup_routers()`;
  ...
  - `reply.py` — хендлеры нижней клавиатуры (`keyboards/reply.py`), 7 кнопок:
    ...
    / «Тест…» → соответствующий контентный экран через `show_screen()`
    (тексты и клавиатуры переиспользуются из `consultation.py`/`info.py`/
    `quiz_data.py`); «Мои записи» → `open_my_bookings(..., send=True)`,
    «Записаться на сессию» → `open_calendar(..., send=True)` (оба +
    `clear_screen` до и `track_screen` после — новый экран заменяет прежнее
    окно); «Задать вопрос Лане» → текст со ссылкой (`ask_lana_kb()`). Каждый хендлер сперва удаляет
    сообщение с текстом кнопки (`delete_safe`). Все фильтруются
