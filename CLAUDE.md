...
- `ui.py` — хелперы гигиены сообщений, на них держится вся уборка переписки
  ...
  - `show_screen(bot, chat_id, text, reply_markup=None)` — **основной** способ:
    ...
    «исчезает», а не копится). Важно про клавиатуру: сообщение с
    ...
    клавиатуру **не** убирает (она на уровне чата — держится до
    `ReplyKeyboardRemove`/новой клавиатуры), поэтому 7 кнопок остаются видны
    во всех разделах.
...
- `handlers/` — по одному роутеру на файл, собираются в `setup_routers()`;
  ...
  - `start.py` — `/start`, `/id`, возврат в главное меню. `/start` через
    ...
    навигация, inline-меню на `/start` больше нет), пришпиливает пользователя
    в БД. Приветствие создаётся **только** на `/start`. Колбэк `start_menu`
    («В меню»/«Назад» из под-экранов) идёт через `show_screen()` — убирает
    текущий экран и показывает приветствие новым сообщением (единое окно:
    предыдущее исчезает, а не дублируется). `MAIN_MENU_TEXT` — без декоративной
